...
var _ = Describe("manger.Manager", func() {
	...
	Describe("NewOptionsFromComponentConfig", func() {
		...
		It("should return config files values for options values", func() {
			...
			Expect(opts.SyncPeriod).To(Equal(&cfg.SyncPeriod.Duration))
			Expect(opts.LeaderElection).To(Equal(*cfg.LeaderElection.LeaderElect))
			Expect(opts.LeaderElectionNamespace).To(Equal(cfg.LeaderElection.ResourceNamespace))
			Expect(opts.LeaderElectionID).To(Equal(cfg.LeaderElection.ResourceName))
			Expect(opts.LeaseDuration).To(Equal(&cfg.LeaderElection.LeaseDuration.Duration))
			Expect(opts.RenewDeadline).To(Equal(&cfg.LeaderElection.RenewDeadline.Duration))
			Expect(opts.RetryPeriod).To(Equal(&cfg.LeaderElection.RetryPeriod.Duration))
			Expect(opts.Namespace).To(Equal(cfg.Namespace))
			Expect(opts.MetricsBindAddress).To(Equal(cfg.MetricsBindAddress))
			Expect(opts.HealthProbeBindAddress).To(Equal(cfg.Health.HealthProbeBindAddress))
			Expect(opts.ReadinessEndpointName).To(Equal(cfg.Health.ReadinessEndpointName))
			Expect(opts.LivenessEndpointName).To(Equal(cfg.Health.LivenessEndpointName))
			Expect(opts.Port).To(Equal(*cfg.Port))
			Expect(opts.Host).To(Equal(cfg.Host))
			Expect(opts.CertDir).To(Equal(cfg.CertDir))
		})
